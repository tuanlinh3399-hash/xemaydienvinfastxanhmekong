...
export default function ProductLeadForm({ productName }: ProductLeadFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatus('success');
            setFormData({ full_name: '', phone: '', branch: currentBranch.name, notes: '' });
        } catch (error) {
    ...
    if (status === 'success') {
        return (
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 flex flex-col items-center justify-center text-center animate-fade-in">
    ...
    return (
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-vinfast-blue/20">
            <div className="mb-6">
                <h3 className="text-2xl font-bold text-vinfast-blue mb-2">Nhận Tư Vấn Ngay</h3>
                <p className="text-gray-600 text-sm">Điền thông tin để nhận báo giá lăn bánh và chương trình khuyến mãi cho xe <strong className="text-gray-800">{productName}</strong>.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Cơ sở tư vấn <span className="text-red-500">*</span></label>
                    <select
                        value={formData.branch}
                        onChange={e => setFormData({ ...formData, branch: e.target.value })}
                        className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-vinfast-blue outline-none transition-all text-gray-800 font-medium cursor-pointer"
                        required
                    >
                        {BRANCH_LIST.map((b) => (
                ...
                <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-vinfast-blue text-white py-3.5 px-6 rounded-xl font-bold hover:bg-blue-800 transition-colors flex items-center justify-center gap-2 mt-2 shadow-md hover:shadow-lg disabled:bg-blue-300"
                >
                    {loading ? (
                        <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    ) : (
                        <>Đăng Ký Nhận Báo Giá <Send size={18} className="ml-1" /></>
                    )}
                </button>
            </form>
        </div>
