...
export default function StickyActionBar({ phone: propPhone, zaloUrl: propZaloUrl }: StickyActionBarProps) {
    const { settings } = useSiteSettings();
    const [isVisible, setIsVisible] = useState(false);

    const fallbackPhone = "0907697036";
    const hotline = propPhone || settings?.hotline || settings?.phone || fallbackPhone;
    const cleanHotline = hotline.replace(/\s+/g, '');
    const zaloUrl = propZaloUrl || settings?.zalo_link || `https://zalo.me/${cleanHotline}`;
