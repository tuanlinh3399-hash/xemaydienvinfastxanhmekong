...
export async function generateMetadata({ params }: PromotionDetailPageProps): Promise<Metadata> {
    ...
    const { data: promotion } = await supabase
        ...
        .select(`
            *,
            desktop_image:media!banner_media_id(url)
        `)
        .eq('slug', slug)
        .single();
    ...
    let imageUrl = (promotion.desktop_image as any)?.url || `https://vinfastxanhmekong.com/images/promotions/${promotion.slug}.webp`;
...
export default async function PromotionDetailPage({ params }: PromotionDetailPageProps) {
    ...
    const { data: promotion, error } = await supabase
        ...
        .select(`
            *,
            desktop_image:media!banner_media_id(url),
            mobile_image:media!mobile_media_id(url)
        `)
        .eq('slug', slug)
        .single();
    ...
    const desktopImageUrl = getFullUrl((promotion.desktop_image as any)?.url, promotion.slug);
    const mobileImageUrl = getFullUrl((promotion.mobile_image as any)?.url || (promotion.desktop_image as any)?.url, promotion.slug);
    ...
    return (
        <div className="bg-vinfast-gray min-h-screen pb-32 md:pb-20">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 flex items-center gap-3 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-40">
                ...
                <a href="tel:0899001177" className="flex-[2] bg-vinfast-blue text-white py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg active:bg-blue-800">
