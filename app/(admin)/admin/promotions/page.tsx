...
type Promotion = {
    ...
    end_date?: string;
    is_active: boolean;
    desktop_image?: MediaObj;
    mobile_image?: MediaObj;
};
...
export default function AdminPromotionsPage() {
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            let query = supabase
                .from('promotions')
                .select('*, desktop_image:media!banner_media_id(*), mobile_image:media!mobile_media_id(*)', { count: 'exact' })
    ...
    const uploadAndInsertMedia = async (file: File, folder: string): Promise<string> => {
    ...
    const openModal = (promo?: Promotion) => {
        ...
        if (promo) {
            ...
            if (promo.desktop_image?.url) setDesktopPreview(promo.desktop_image.url);
            if (promo.mobile_image?.url) setMobilePreview(promo.mobile_image.url);
        } else {
            setEditing(null);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            let bannerId = editing?.banner_media_id || null;
            let mobileId = editing?.mobile_media_id || null;

            if (desktopFile) {
                bannerId = await uploadAndInsertMedia(desktopFile, 'promotions');
            }
            if (mobileFile) {
                mobileId = await uploadAndInsertMedia(mobileFile, 'promotions');
            }
                        ...
                        <table className="w-full text-left border-collapse whitespace-nowrap min-w-[800px]">
                            ...
                            <tbody className="divide-y divide-gray-200 text-sm text-gray-700">
                                {items.length === 0 ? (
                                    ...
                                    items.map((item) => {
                                        const thumbUrl = item.desktop_image?.url || '/images/placeholder.webp';
                                        return (
